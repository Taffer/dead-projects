...
vector<Node>* Board::neighbours_for(unsigned x, unsigned y)
{
    ...
    //   bot-left, bot-right
    //
    // We need to check each potential neighbour for underflow/overflow
...
// Return a vector of the specified player's positions.
//
// Caller must delete.
vector<Node>* Board::played_locations(Chits who)
{
	vector<Node>* played = new vector<Node>;

	// This would be pretty inefficient for large boards; AI player clients
	// could keep track of their own moves instead of calling this repeatedly.
    for(unsigned i = 0; i < _size; i++) {
        for(unsigned j = 0; j < _size; j++) {
            if(_chits[i][j] == who) {
            	played->push_back(Node(i, j));
            }
        }
    }
...
// Print indent for board representation.
